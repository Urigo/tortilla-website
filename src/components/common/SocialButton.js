...
class ShareButton extends React.Component {
  ...
  renderFacebook() {
    return (
      <Container className={`resp-sharing-button__link ${this.props.className}`} scale={this.props.scale} onClick={this.openShareWindow} href={this.state.shareUrl} target="_blank">
  ...
  getShareUrl = (media = this.props.media) => {
    switch (this.props.media) {
      case 'facebook': return `https://facebook.com/sharer/sharer.php?u=${escape(this.state.url)}`
      case 'twitter': return `https://twitter.com/intent/tweet/?text=${escape(this.state.text)}&url=${escape(this.state.url)}`
      case 'pinterest': return `https://pinterest.com/pin/create/button/?url=${escape(this.state.url)}&media=${escape(this.state.url)}&description=${escape(this.state.text)}`
      default: return ''
    }
  }

  openShareWindow = () => {
    window.open(this.state.shareUrl, `Share on ${this.props.media}`, 'width=512,height=1024')
  }
}
