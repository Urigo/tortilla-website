...
class ShareButton extends React.Component {
  ...
  render() {
    ...
    switch (state.media) {
      case 'facebook':
        return this.renderFacebook(state)
      case 'twitter':
        return this.renderTwitter(state)
      case 'pinterest':
        return this.renderPinterest(state)
      default:
        return null
    }
  }
  ...
  renderFacebook(state) {
    return (
      <Container
        className={`resp-sharing-button__link ${state.className}`}
        scale={state.scale}
        onClick={() => this.openShareWindow(state)}
        href={state.shareUrl}
        target="_blank"
      >
  ...
  getShareUrl(state) {
    switch (state.media) {
      case 'facebook':
        return `https://facebook.com/sharer/sharer.php?u=${escape(state.url)}`
      case 'twitter':
        return `https://twitter.com/intent/tweet/?text=${escape(
          state.text
        )}&url=${escape(state.url)}`
      case 'pinterest':
        return `https://pinterest.com/pin/create/button/?url=${escape(
          state.url
        )}&media=${escape(state.url)}&description=${escape(state.text)}`
      default:
        return ''
    }
  }
